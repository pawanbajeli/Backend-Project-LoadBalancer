...
const port = 5001; // Set the port number

app.use(express.json()); // Middleware to parse JSON request bodies

app.get('/', (req, res) => {
    res.send('Welcome to API1'); // Respond with a welcome message for the root route
});
...
app.get('/rest', async (req, res) => {
    console.log('API1 received a REST request'); // Log the receipt of a REST request
    const responseTime = Math.random() * 2000; // Random delay up to 2000ms
...
app.get('/graphql', async (req, res) => {
    console.log('API1 received a GraphQL request'); // Log the receipt of a GraphQL request
    const responseTime = Math.random() * 2000; // Random delay up to 2000ms
    await delay(responseTime); // Simulate delay
...
app.get('/grpc', async (req, res) => {
    console.log('API1 received a gRPC request'); // Log the receipt of a gRPC request
    const responseTime = Math.random() * 2000; // Random delay up to 2000ms
    await delay(responseTime); // Simulate delay
...
app.listen(port, () => {
    console.log(`API1 is running on http://localhost:${port}`); // Log the server start message
});
